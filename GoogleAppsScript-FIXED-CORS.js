 ...
 * This version explicitly handles CORS to prevent blocking
 */

const PROCEDURE_SHEET_NAME = 'Sheet1';
const DC_SHEET_NAME = 'DCs';

// DC Column indices
const DC_COLS = {
  ID: 0, HOSPITAL_NAME: 1, DC_NO: 2, MATERIAL_TYPE: 3, SAVED_AT: 4,
  RECEIVED_BY: 5, REMARKS: 6, STATUS: 7, ITEMS: 8, INSTRUMENTS: 9,
  BOX_NUMBERS: 10, RETURNED_BY: 11, RETURNED_AT: 12, RETURNED_REMARKS: 13,
  INVOICE_REF: 14, INVOICE_REMARKS: 15, CASH_AT: 16, CASH_AMOUNT: 17,
  CASH_REMARKS: 18, HISTORY: 19
};
...
function appendDC(data) {
  ...
  const rowData = [
    data.id || '', data.hospitalName || '', data.dcNo || '',
    data.materialType || '', data.savedAt || '', data.receivedBy || '',
    data.remarks || '', data.status || 'pending', itemsJson,
    instrumentsJson, boxNumbersJson, data.returnedBy || '',
    data.returnedAt || '', data.returnedRemarks || '', data.invoiceRef || '',
    data.invoiceRemarks || '', data.cashAt || '', data.cashAmount || '',
    data.cashRemarks || '', historyJson
  ];
...
function getDCs() {
  ...
  const dcs = values.map(row => {
    let items = [], instruments = [], boxNumbers = [], history = [];
    
    try { items = row[DC_COLS.ITEMS] ? JSON.parse(row[DC_COLS.ITEMS]) : []; } catch (e) {}
    try { instruments = row[DC_COLS.INSTRUMENTS] ? JSON.parse(row[DC_COLS.INSTRUMENTS]) : []; } catch (e) {}
    try { boxNumbers = row[DC_COLS.BOX_NUMBERS] ? JSON.parse(row[DC_COLS.BOX_NUMBERS]) : []; } catch (e) {}
    try { history = row[DC_COLS.HISTORY] ? JSON.parse(row[DC_COLS.HISTORY]) : []; } catch (e) {}
...
function updateDC(data) {
  ...
  if (lastRow <= 1) throw new Error('No DCs found');
  ...
  if (rowIndex === -1) throw new Error('DC not found with id: ' + data.id);
